    ...
    <List.Accordion id={mealPlan} key={mealPlan} title={mealPlan}>
      {data.map((food, index) => (
        <List.Item
          ...
          right={() => <Ionicons style={{ backgroundColor: globalStyles.colors.SecondaryButton, borderRadius: 10, paddingHorizontal: 10 }} color={'red'} name="trash-bin" size={20} onPress={() => handleDeleteFood(mealPlan, food)} />}
        />
      ))}
    </List.Accordion>
  )
